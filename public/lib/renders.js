...
function drawBasicPetals(ctx = _ctx, index, rarity, size = 1, color = colors.white) {
    ...
    setStyle(ctx, color, count > 1 ? 0.2 : 0.25);
...
function trianglePetal(ctx = _ctx, hit = false, col = colors.white) {
    setStyle(ctx, mixColors(col, "#FF0000", hit * 0.5), 0.6, .2);
    ctx.beginPath();
    polygon(ctx, 3, 1, 0);
    ctx.fill();
    ctx.stroke();
...
function drawHeavy(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.stingerBlack, "#FF0000", hit * 0.5), 0.25);
    ctx.beginPath();
    ctx.arc(0, 0, 1, 0, TAU);
    ctx.closePath();
    ctx.fill();
...
function drawRice(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.white, "#FF0000", hit * 0.5), 0.8);
    ctx.rotate(Math.PI / 4);

    ctx.beginPath();
    ctx.moveTo(-0.7, 0.1);
    ctx.quadraticCurveTo(0, -0.4, 0.7, 0.1);
    ctx.stroke();
...
function drawCactusPetal(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.cactusGreen, "#FF0000", hit * 0.5), 0.225);
...
function drawBone(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(mixColors(colors.white, "#000000", 0.2), "#FF0000", hit * 0.5), 0.25);
...
function drawDirt(id, ctx = _ctx, hit = false) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const dist = 1 + Math.sin(id * 5 + i * 7) * 0.125;
...
function drawMagnolia(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.ladybugRed, "#FF0000", hit * 0.5), 0.25);
...
function drawCorn(ctx = _ctx, hit = false) {
    ...
    ctx.beginPath();
    ctx.moveTo(0.9, 0.8);
    ctx.quadraticCurveTo(1.4, 0, 0.9, -0.8);
    ctx.bezierCurveTo(0.55, -1.25, -0.05, -0.95, -0.875, -0.55);
    ctx.quadraticCurveTo(-0.1, 0, -0.875, 0.55);
    ctx.bezierCurveTo(-0.05, 0.95, 0.55, 1.25, 0.9, 0.8);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
}
...
function drawOrange(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.orange, "#FF0000", hit * 0.5), 0.25);
    ...
    setStyle(ctx, mixColors(colors.leafGreen, "#FF0000", hit * 0.5), 0.25);
...
function drawYinYang(ctx = _ctx, hit = false) {
    ...
    const YINYANG_WB = mixColors("#dedede", "#FF0000", hit * 0.5);
    const YINYANG_B = mixColors("#3d3d3d", "#FF0000", hit * 0.5);
...
function drawHoney(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.honeyGold, "#FF0000", hit * 0.5), 0.3);
    dipPolygon(ctx, 6, 1, 0, 0);
    ctx.fill();
    ctx.stroke();
}
...
function drawPincer(ctx = _ctx, hit = false) {
    ...
    ctx.beginPath();
    ctx.moveTo(-1, 0);
    ctx.quadraticCurveTo(-0.3, -1.25, 1, 0.3);
    ctx.lineTo(1, 0.3);
    ctx.quadraticCurveTo(0.3, -0.15, -1, 0);
    ctx.closePath();
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
...
function drawStick(color, ctx = _ctx, hit = false) {
    let darkColor = mixColors(color, "#FF0000", hit * 0.5);
    darkColor = mixColors(darkColor, "#000000", 0.2);
    setStyle(ctx, darkColor, 0.65);
...
function drawPrimrose(ctx = _ctx, hit = false, real = false) {
    ...
    ctx.lineWidth = 0.2;
    const step = TAU / 5;
    ctx.fillStyle = mixColors("#fff9ce", "#FF0000", hit * 0.5);
    ctx.strokeStyle = mixColors(ctx.fillStyle, "#000000", 0.2);

    ctx.beginPath();
    ctx.moveTo(0, 0);
    for (let i = 0; i < 5; i++) {
        const angle = i * step;
        const a1 = angle - step * 0.5;
        const b1 = angle + step * 0.5;
    ...
    ctx.fillStyle = mixColors("#fff185", "#FF0000", hit * 0.5);
    ctx.beginPath();
    ctx.arc(0, 0, 0.25, 0, TAU);
    ctx.fill();
}
...
function drawSpellbook(ctx = _ctx, hit = false, real = false, spell = 0) {
    ...
    ctx.strokeStyle = mixColors(mixColors(colors.book, "#000000", 0.2), "#FF0000", hit * 0.5);
    ctx.lineWidth = 0.2;
    ctx.beginPath();
    ctx.moveTo(-1, -1);
    ctx.lineTo(-1, 1);
...
function drawDeity(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.lightningTeal, "#FF0000", hit * 0.5), 0.15);
...
function drawYucca(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.peaGreen, "#FF0000", hit * 0.5), 0.25);
    ctx.beginPath();
    ctx.moveTo(1.1, 0);
...
export function drawCutter(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.stingerBlack, "#FF0000", hit * 0.5), 0.2, 0);

    ctx.beginPath();
    dipPolygon(ctx, 13, 1, 1, Math.PI / 6);
...
function drawJelly(ctx = _ctx, hit = false) {
    ...
    ctx.globalAlpha = 0.5;
    ctx.fill();
    ctx.strokeStyle = mixColors(colors.jellyPink, "#FF0000", hit * 0.5);
    ctx.globalAlpha = 1;
    ctx.stroke();

    ctx.fillStyle = mixColors(colors.jellyPink, "#FF0000", hit * 0.5);
    ctx.globalAlpha = 1;
    ctx.beginPath();
...
function drawSpongePetal(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.sponge, "#FF0000", hit * 0.5), 0.25);

    dipPolygon(ctx, 7, 0.9, -4, 0);
    ctx.fill();
    ctx.stroke();
}
...
function drawPearl(ctx = _ctx, hit = false) {
    ...
    ctx.fillStyle = colors.white;
    ctx.beginPath();
    ctx.ellipse(0.3, -0.3, 0.45, 0.3, Math.PI / 4, 0, Math.PI * 2);
    ctx.closePath();
    ctx.fill();
}

function drawShellPetal(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.peach, "#FF0000", hit * 0.5), 0.25);
    // Body
    ctx.beginPath();
...
function drawStarfishPetal(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors(colors.starfish, "#FF0000", hit * 0.5), 0.25);

    ctx.beginPath();
    ctx.moveTo(-1.2, -0.5);
    ctx.quadraticCurveTo(0,-.5,1,-0.4);
    ctx.arc(.99, 0, 0.3999, -Math.PI / 2, Math.PI / 2);
    ctx.lineTo(1, 0.4);
    ctx.quadraticCurveTo(0,.5,-1.2,0.55);
    ctx.quadraticCurveTo(-1.35, 0.2, -1.3, 0.3);
    ctx.quadraticCurveTo(-1.4, 0, -1.3, -0.3);
    ctx.quadraticCurveTo(-1.25, -0.45, -1.2, -0.5);
    ctx.fill();
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.arc(-0.45, 0, 0.3, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(0.25, 0, 0.25, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(0.825, 0, 0.2, 0, Math.PI * 2);
    ctx.fill();
}
...
function drawFangPetal(ctx = _ctx, hit = false) {
    ...
    ctx.moveTo(-1, 0);
    ctx.lineTo(-0.3, 0.3);
    ctx.quadraticCurveTo(0, 0.5, .3, .3);
    ctx.lineTo(0.3, 0.3);
    ctx.lineTo(1, 0);
    ctx.lineTo(0.3, -0.3);
    ctx.quadraticCurveTo(0, -0.5, -.3, -.3);
...
function drawBattery(ctx = _ctx, hit = false) {
    setStyle(ctx, mixColors("#C8C8C8", "#FF0000", hit * 0.5), 0.25);
    ctx.fillRect(-0.25, -1, 0.5, 0.5);
    ctx.strokeRect(-0.25, -1, 0.5, 0.5);

    setStyle(ctx, mixColors("#323233", "#FF0000", hit * 0.5), 0.25);
    ctx.fillRect(-0.6, -0.7, 1.2, 1.7);
    ctx.strokeRect(-0.6, -0.7, 1.2, 1.7);

    setStyle(ctx, mixColors("#C98A5B", "#FF0000", hit * 0.5), 0.25);
...
function drawDust(ctx = _ctx, index, rarity) {
    let count = state.petalConfigs[index].tiers[rarity].count;
    setStyle(ctx, colors.rockGray, 0.25);
...
function drawShrubPetal(color, ctx = _ctx, hit = false) {
    ...
    ctx.fillStyle = mixColors(ctx.fillStyle, "#FFFFFF", .065)
...
function petalRender(index, hit, ctx, id, size) {
    ...
    switch (index) {
        ...
        case 3: // Heavy
            ...
            break;
        case 4: // Stinger
            trianglePetal(ctx, hit, colors.lighterBlack);
            break;
        case 5: // Rice
            drawRice(ctx, hit);
            break;
        ...
        case 29: // Peas
            ...
            break;
        case 30: // Stick
            drawStick(colors.stick, ctx, hit);
            break;
        ...
        case 56: // Battery
            ...
            break;
        case 57: // Dust
            setStyle(ctx, mixColors(colors.rockGray, "#FF0000", hit * 0.5), 0.35);
            polygon(ctx, 6, 1, performance.now() * 0.001 + id);
            ctx.fill();
            ctx.stroke();
            break;
        case 58: // Armor
...
function drawLadybug(id, color, hit = false, ctx = _ctx, index, rarity) {
    ...
    // Spots
    ctx.fillStyle = black;
    let amount = (2 + Math.abs(Math.sin(id) * 0.667) * 12) | 0;
    amount += Math.floor(rarity / 1.5);

    for (let i = 0; i < amount; i++) {
        let angle = Math.sin(i * 100 + id) * TAU;
        let radius = Math.sin(i * 1000 + id) * 0.2 + 0.2;
...
function drawRock(id, color, rarity, hit = false, ctx = _ctx) {
    ...
    setStyle(ctx, mixColors(color, "#FF0000", hit * 0.5), 0.2 / (rarity * 0.1 + 1));
    ctx.beginPath();
...
function drawBeetle(id, color, hit = false, ctx = _ctx, attack = false, date) {
    ...
    setStyle(ctx, mixColors(colors.stingerBlack, "#FF0000", hit * 0.5), 0.15);
    ctx.save();
...
function drawRoach(hit = false, friend, ctx = _ctx) {
    setStyle(ctx, mixColors(friend ? colors.playerYellow : colors.roachHead, "#FF0000", hit * 0.5), 0.14);
    ctx.beginPath();
    ...
    setStyle(ctx, mixColors(friend ? colors.playerYellow : colors.roach, "#FF0000", hit * 0.5), 0.14);
    ctx.beginPath();
...
function drawScorpion(id, color, hit = false, ctx = _ctx, date) {
    ...
    setStyle(ctx, mixColors(color, "#FF0000", hit * 0.5), 0.13);
    ...
    // Stinger
    setStyle(ctx, mixColors(colors.stingerBlack, "#FF0000", hit * 0.5), 0.125, 0);
    ctx.beginPath();
    ctx.moveTo(-0.1, 0);
...
function drawTermiteMound(hit, friend, ctx = _ctx) {
    setStyle(ctx, mixColors(friend ? colors.playerYellow : colors.scorpionBrown, "#FF0000", hit * 0.5), 0.25);
    ctx.beginPath();
...
const spongeColors = [colors.sponge, colors.rosePink, colors.diepPentagon, colors.diepTriangle];

function drawSponge(id, friendly, hit = false, ctx = _ctx) {
    ctx.save();
    setStyle(ctx, mixColors(friendly ? colors.playerYellow : spongeColors[id % 4], "#FF0000", hit * 0.5), 0.125);

    ctx.scale(1.1, 1.1);
    ctx.beginPath();
    dipPolygon(ctx, 15, .8, -8, 0);
    ctx.fill();
    ctx.stroke();
    ...
    for (let i = 0; i < 5; i++) {
        ...
        ctx.beginPath();
        ctx.arc(Math.cos(angle) * 0.51, Math.sin(angle) * 0.51, 0.15, 0, TAU);
        ctx.closePath();
        ctx.fill();
        ctx.beginPath();
        ctx.arc(Math.cos(angle) * 0.26, Math.sin(angle) * 0.26, 0.1, 0, TAU);
        ctx.closePath();
        ctx.fill();
        ctx.beginPath();
        ctx.arc(Math.cos(angle) * 0.1, Math.sin(angle) * 0.1, 0.06, 0, TAU);
        ctx.closePath();
        ctx.fill();
    }
    ctx.restore();
...
function drawLeechRender(ctx = _ctx) {
    ...
    ctx.moveTo(0.8, -1);
    ctx.quadraticCurveTo(0.8, -1.125, 0.75, -1.2);
    ctx.stroke();

    ctx.beginPath();
    ctx.moveTo(0.65, -0.85);
    ctx.quadraticCurveTo(0.6, 0.6, -0.7, 0.9);
    ctx.strokeStyle = stroke;
    ctx.lineWidth = 0.5;
    ctx.stroke();
...
function drawFlyWings(x, y, s, id, rotSpd, hit = false, ctx = _ctx, date) {
    ...
    ctx.translate(x, y);
    ctx.scale(s, s);
    ctx.fillStyle = mixColors("#FFFFFF", "#FF0000", hit * 0.7);
...
function drawFirefly(ctx = _ctx, friendly, id, hit = false, date) {
    setStyle(ctx, mixColors(mixColors(colors.playerYellow, "#FFFFFF", 0.2), "#FF0000", hit * 0.5), 0.275);
    ctx.beginPath();
    ctx.ellipse(-1, 0, 0.8 * 1.25, 0.75 * 1.25, 0, 0, TAU);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();

    setStyle(ctx, mixColors(mixColors(colors.playerYellow, "#FFFFFF", 0.6), "#FF0000", hit * 0.5), 0.275);
    ctx.beginPath();
    ctx.arc(-1, 0, 0.525, 0, TAU);
    ctx.closePath();
    ctx.fill();
...
function drawWasp(id, color, altColor, hit = false, ctx = _ctx, date) {
    const myColor = mixColors(color, "#FF0000", hit * 0.5);
    const myAltColor = mixColors(altColor, "#FF0000", hit * 0.5);
    setStyle(ctx, myAltColor, 0.15, 0);
    ...
    ctx.beginPath();
    ctx.moveTo(.275, -.625);
    ctx.quadraticCurveTo(.125, 0, .275, .625);
    ctx.moveTo(.15, -.6);
    ctx.quadraticCurveTo(0, 0, .15, .6);
    setStyle(ctx, myAltColor, 0.2, 0);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(-.2, -.65);
    ctx.quadraticCurveTo(-.35, 0, -.2, .65);
    ctx.moveTo(-.4, -.6);
    ctx.quadraticCurveTo(-.55, 0, -.4, .6);
    setStyle(ctx, myAltColor, 0.2, 0);
    ctx.stroke();
    ctx.restore();
...
function drawShrub(id, color, ctx = _ctx, hit = false) {
    ...
    dipPolygon(ctx, 9, 1.2, -2, 0)
    ctx.fill()
    ctx.stroke()

    ctx.fillStyle = mixColors(ctx.fillStyle, "#FFFFFF", .055)
    dipPolygon(ctx, 8, .8, -2, 0)
    ctx.fill()

    ctx.fillStyle = mixColors(ctx.fillStyle, "#FFFFFF", .055)
    dipPolygon(ctx, 7, .5, -2, 0)
    ctx.fill()
}
...
const crabBodyColors = [colors.crabBodyOrange];
